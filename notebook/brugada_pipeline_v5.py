# =============================================================================
# BRUGADA SYNDROME DETECTION — COMPLETE ML PIPELINE (Version 5)
# IDSC 2026 | Dataset: Brugada-HUCA (PhysioNet)
# Author 1: Nur Insyirah Binti Azhan
# Author 2: Muhammad Zuhairi Bin Ahmad Rizal
# Full feature vector per patient (v5):
...
N         = len(meta)
N_brugada = (meta['label'] == 1).sum()   
N_normal  = (meta['label'] == 0).sum()   
...
def extract_patient_features(patient_id, db_name=DB_NAME, fs=FS):
    ...
    try:
        ...
        v1_filt = butterworth_bandpass(record.p_signal[:, V1_IDX], fs=fs)
        r_peaks = pan_tompkins_rpeaks(v1_filt, fs=fs)

        if len(r_peaks) < 2:
            print(f"  Warning: fewer than 2 R-peaks for {pid} — skipped.")
            return None

        beats = [extract_features_one_beat(v1_filt, r, fs) for r in r_peaks]
        beats = [b for b in beats if b is not None]

        if not beats:
            print(f"  Warning: no valid beats for {pid} — skipped.")
            return None

        arr = np.array(beats)   # (n_beats, 4)
        return tuple(float(np.mean(arr[:, i])) for i in range(4))
...
for idx, row in meta.iterrows():
    ...
    if features is not None:
        ...
        labels.append(label)
        pids.append(pid)
        print(f"x1={features[0]:+.3f}  x2={features[1]:5.1f}ms  "
              f"x3={features[2]:+.3f}  x4={features[3]:+.3f}")
    else:
        print("SKIPPED")
...
feature_df.to_csv('feature_matrix_v5.csv', index=False)
print("Saved: feature_matrix_v5.csv")
...
summary = feature_df.groupby('label')[
    ['x1_j_elevation', 'x2_qrs_duration', 'x3_t_amplitude', 'x4_r_s_drop']
].agg(['mean', 'std']).round(4)
print(summary)

# Clinical direction check
x1_b = feature_df[feature_df['label']==1]['x1_j_elevation'].mean()
x1_n = feature_df[feature_df['label']==0]['x1_j_elevation'].mean()
x2_b = feature_df[feature_df['label']==1]['x2_qrs_duration'].mean()
x2_n = feature_df[feature_df['label']==0]['x2_qrs_duration'].mean()
x3_b = feature_df[feature_df['label']==1]['x3_t_amplitude'].mean()
x4_b = feature_df[feature_df['label']==1]['x4_r_s_drop'].mean()
x4_n = feature_df[feature_df['label']==0]['x4_r_s_drop'].mean()
...
# =============================================================================
# STEP 5 — NESTED RandomizedSearchCV + 5-FOLD OUTER CV
# =============================================================================

print("\n" + "=" * 65)
print("STEP 5 — Nested RandomizedSearchCV + 5-Fold Outer CV")
print("=" * 65)
print(f"\nSearch space:")
print(f"  C     : log-uniform [0.01, 1000]")
print(f"  gamma : log-uniform [0.0001, 10]")
print(f"  Random combinations per outer fold : {N_ITER_SEARCH}")
print(f"  Inner folds                        : 5")
print(f"  Outer folds                        : 5")
print(f"  Total SVM fits : {N_ITER_SEARCH} x 5 x 5 = {N_ITER_SEARCH*5*5}\n")
...
# RandomizedSearchCV wraps pipeline with inner CV
...
for fold_num, (train_idx, test_idx) in enumerate(outer_cv.split(X, y), 1):
    ...
    y_pred_fold = rand_search.predict(X_test)
    y_prob_fold = rand_search.predict_proba(X_test)[:, 1]

    if len(np.unique(y_test)) > 1:
        raw_auc = roc_auc_score(y_test, y_prob_fold)
        if raw_auc < 0.5:
            print(f"  Fold {fold_num}: AUC={raw_auc:.4f} < 0.5 — "
                  f"flipping predictions (model had labels inverted)")
    ...
    best_params_list.append({
        'fold': fold_num, 'C': round(best_C, 4), 'gamma': round(best_gamma, 4)
    })

    print(f"  Fold {fold_num}: AUC={fold_auc:.4f}  "
          f"best_C={best_C:.4f}  best_gamma={best_gamma:.6f}  "
...
ax6.axhline(y=np.mean(fold_aucs), color='orange', linestyle='--',
            linewidth=2.0, label=f'Mean = {np.mean(fold_aucs):.3f}')
...
plt.savefig('brugada_results_v5.png', dpi=150, bbox_inches='tight')
plt.show()
print("Saved: brugada_results_v5.png")
...
# =============================================================================
print("\n" + "=" * 65)
print("PIPELINE COMPLETE — v5 Summary")
print("=" * 65)
print(f"  Patients processed : {len(y)} / {N}")
print(f"  Patients skipped   : {N - len(y)} (insufficient signal quality)")
print(f"  Feature matrix     : {X.shape}  (saved: feature_matrix_v5.csv)")
print(f"  Results figure     : brugada_results_v5.png")
...
print(f"\nv5 changes vs v4:")
print(f"  FIX 1 — x_2: QRS Duration (ms) replaces S-to-ST slope")
print(f"  FIX 2 — AUC inversion protection (fold collapse fix)")
print(f"  FIX 3 — Inner CV: 5-fold (was 3-fold)")
print(f"  FIX 4 — RandomizedSearchCV with log-uniform C/gamma sampling")
